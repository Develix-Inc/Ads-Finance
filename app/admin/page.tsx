...
import {
  adminGetAllUsers, adminGetAllPayments, adminGetWithdrawals,
...
import {
  ...
  FaRightFromBracket, FaShieldHalved, FaPen, FaX
...
export default function AdminDashboard() {
  ...
  async function rejectPayment(p: any) {
    ...
    if (note !== undefined) {
      await adminRejectPayment(p.id, p.uid, note, admin.email);
      load();
    }
  }
  ...
  async function saveUserBalance(uid: string) {
    const val = parseFloat(editBal);
    if (isNaN(val)) return;
    await adminCreditUser(uid, val, "Admin balance adjustment", admin.email);
    Swal.fire({ ...SWAL, icon:"success", title:"Balance Updated", timer:1000, showConfirmButton:false });
    setEditBal(""); load();
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200">
      ...
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between bg-slate-900/60 sticky top-0 z-40 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-black text-white text-base leading-none">AdsFinance Admin</p>
            <p className="text-[10px] text-teal-400 font-mono tracking-widest uppercase">Control Panel</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6">
        ...
        {tab==="overview" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label:"Active Nodes",    value:activeNodes,   color:"violet"},
              { label:"Pending Payments",value:pendingPay,    color:"amber" },
              { label:"Pending Withdrawals",value:pendingWit, color:"rose"  },
            ].map((s,i) => (
              <div key={i} className="bg-slate-900 border border-white/10 rounded-[20px] p-6">
                ...
                <p className={`text-4xl font-black ${s.color==="teal"?"text-teal-400":s.color==="violet"?"text-violet-400":s.color==="amber"?"text-amber-400":"text-rose-400"}`}>{s.value}</p>
              </div>
            ))}
                    ...
                    {users.map((u,i) => (
                      <tr key={i} className="hover:bg-white/3 transition-colors">
                        <td className="px-5 py-4">
                          ...
                          <p className="text-slate-500 text-xs mt-0.5 capitalize">{u.investmentGoal?.replace("_"," ") || "—"}</p>
                        </td>
                        <td className="px-5 py-4">
                ...
                <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
                  ...
                  <motion.div initial={{y:30,opacity:0}} animate={{y:0,opacity:1}} exit={{y:30,opacity:0}}
                    ...
                    {[
                      ...
                      { k:"Investment Goal", v: selectedUser.investmentGoal?.replace("_"," ")||"—" },
                    ].map(r => (
                    ...
                    <div className="mt-5 space-y-3">
                      <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Update Wallet Balance</p>
                      <div className="flex gap-2">
                        <input value={editBal} onChange={e=>setEditBal(e.target.value)}
                          placeholder={fmt(selectedUser.walletBalance??0)}
                          className="flex-1 bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-teal-500 transition-colors" />
                        <button onClick={() => saveUserBalance(selectedUser.uid)}
                          className="px-4 py-2.5 bg-teal-500 hover:bg-teal-400 text-slate-950 font-black rounded-xl text-sm transition-colors">
                          Save
                        </button>
                      </div>
                    </div>
        ...
        {tab==="payments" && (
          <div className="space-y-3">
            <p className="text-slate-400 text-sm">{pendingPay} pending · {payments.length} total</p>
            <div className="space-y-3">
              {payments.map((p,i) => (
        ...
        {tab==="withdrawals" && (
          <div className="space-y-3">
            <p className="text-slate-400 text-sm">{pendingWit} pending · {withdrawals.length} total</p>
            <div className="space-y-3">
              {withdrawals.map((w,i) => (
                <div key={i} className="bg-slate-900 border border-white/10 rounded-[20px] p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
